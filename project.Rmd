...
Since the 1960s, the US has recognized the damage pollution causes, and has been passing environmental acts ever since in an attempt to reduce harmful emmisions. Over the last couple decades, the awareness for the dangers of pollution has risen even higher, especially with the threat of global warming. Data measuring the concentration of pollutants has allowed researchers to realize the harmful affects that pollutants have on health, and those dangers have been communicated to the public. Weather channels are even seen reporting the air quality and indicate if it is safe to exercise outside. However, despite the increased awareness, it seems like the US is only producing more cars and more people are commuting to work, not to mention the rising population. Furhermore, people are shopping less in stores, instead choosing to order online and have the items delivered, often from another state. Are the environmental restrictions and improvements in pollutant filtering for fuel-burning processes enough to improve air quality? Or has pollution been on the rise within the US?

We argue that over time, pollution levels have improved within the US for metro and micropolitan areas. 


# Data


We pulled Air Quality Index (AQI) data from [Pre-generated data files](https://aqs.epa.gov/aqsweb/airdata/download_files.html). AQI is an indicator of how dangerous current concentrations of a pollutant are. A value less than 50 is good, while over 300 is hazardous. The data is organized by CBSA, which defines metro and micro-politan areas. We use both daily and yerly summaries. 
...
We do not look at every pollutant specifically, but these are the pollutants measured at the testing sites in our datasets:
...
- SO2: Sulfure Dioxide is formed from the burning of coal, oil, and copper smelting. It has similar effects to NO2.

### Data cleaning

- Combined 40 years of daily AQI data, which were in separate files. 


- Combined last 6 years of annual data using, and filtered the data for the 3 counties in Wisconsin that we focused on
...
Pollution data is inherently noisy, as levels can fluctuate widely from day to day. The wind could blow a high level of pollution from one area to another, for example, or there might be a forest fire every few years that increases levels. For this reason, a very small percentage of the variance is explained by the model. However, the downward trend estimate of about -.55 per year (P < .001) has a small standard error. Even noisy data can have a significant trend. While -.55 is a small change in one year, over 40 years it has led to about a 22 point decrease in the average aqi levels which is a significant improvement. If the trend continues, by 2060 the average will be around 17. It is also promising how the variability decreases after 2012. This suggests that extreme AQI values, which cause the most damage, are being minimized.

Some pollutant are easier to reduce in emmisions than others (Fig 1-B). The improvement seen in *Fig 1-A* is most  attributed to CO, SO2, NO2. The US has made little progress in improving the concentraion of particulate matter. The varibiliity of *Fig 1-A* is likely explained by Ozone, as it's variability goes down dramatically after 2012. What is causing this variability in Ozone? 
...
There are many factors that affect AQI values which we did not explore. Given more time, it would be useful to examine more subsets of the data, since there is so much available. The project does not explore 
